...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            if i==0:
                x, cache2=word_embedding_forward(self._start, W_embed) # (N,1,D)
            else:
                x, cache2=word_embedding_forward(captions[:,i], W_embed)  #input x=(N,T), out (N,T,D)
            if self.cell_type == 'rnn':
                h, cache3=rnn_step_forward(np.squeeze(x),prev_h,Wx,Wh,b)    # input X=(N, D), H=(N,H)
            else:
                h, c, _ = lstm_step_forward(np.squeeze(x), prev_h, c, Wx, Wh, b)
                

            
            scores, cache4=temporal_affine_forward(h[:, np.newaxis, :],W_vocab,b_vocab)  # (N,1,H)->(N,1,M)
