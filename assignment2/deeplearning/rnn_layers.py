...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    kernel=np.matmul(prev_h,Wh)+np.matmul(x,Wx)+b
    next_h=np.tanh(kernel)
    cache=x, prev_h,Wx, Wh,kernel
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    #1-tanh^2                                          #
    ##############################################################################
    x,prev_h,Wx, Wh,  kernel=cache
    dtanh=1-np.tanh(kernel)**2
    dkernel=dnext_h*dtanh
    #dtanh is (N,H)
    dprev_h=np.matmul(dkernel,np.transpose(Wh))
    dWh=np.matmul(np.transpose(prev_h),dkernel)
    dx=np.matmul(dkernel,np.transpose(Wx))
    dWx=np.matmul(np.transpose(x),dkernel)
    db=np.sum(dkernel,axis=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    #h, cache = [],{}
    #N,T,D=x.shape
    #x_rnn = x.transpose(1,0,2)
    #h.append(h0)
    #for i in range(T):
       # next_h, cache[i]=rnn_step_forward(x_rnn[i],h[i],Wx,Wh,b)
      #  
     #   h.append(next_h)

    #h= np.asarray(h[1:])  #slect only the 1:end part
   # h=h.transpose(1,0,2)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, Wx, Wh, i, f, o, g, a_i, a_f,a_o,a_g,next_c)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, i, f, o, g, ai, af,ao,ag,next_c=cache
