...
def affine_forward(x, w, b):
  ...
  out =(np.dot(w.T,x_flatten)+b.reshape(b.shape[0],1)).T
  #N = x.shape[0]
  #D = np.prod(x.shape[1:])
  #x2 = np.reshape(x, (N, D))
  #out = np.dot(x2, w) + b
...
def affine_backward(dout, cache):
  ...
  N=x.shape[0]
  x_flatten = x.reshape(x.shape[0], -1).T
  dw =np.dot(x_flatten,dout)
  db = np.sum(dout,axis=0)  # why there is no need to include the 1/N??
  dx = np.dot(dout,w.T)   #1/N
    ...
    for xf in range(F):
      for i in range(H_prime):
        for j in range(W_prime):
            out[xn,xf,i,j]=np.sum(x_padded[xn,:,i*stride:i*stride+HH,j*stride:j*stride+WW]*w[xf,:])+b[xf]

# always remember the python's convention of the matrices
...
def conv_backward_naive(dout, cache):
  ...
  dw = np.zeros_like(w)
  dx = np.zeros_like(x)
  dx_new=np.zeros_like(x_padded)
  db = np.zeros_like(b)


  #dw, cache2= conv_forward_naive(x,dout,bz,conv_param)
    ...
    for f in range(F):
        for k in range(H_prime):
            for l in range(W_prime):
                ...
                dw[f] += (x_padded[n, :, k*stride: k*stride + HH, l*stride:l*stride+ WW] * dout[n, f, k, l]) #x convolve the dout
                dx_new[n, :, k*stride: k*stride+ HH, l*stride:l*stride+ WW] += (w[f, :, ] * dout[n, f, k, l])
  
  # it could be solved also with regard to the destination matrices. If you picture it from the other convolution ideas.
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in range(N):
    for c in range(C):
      for h in range(H_prime):
        for w in range(W_prime):
            #find the max position, could be solved with a max mask layer.
